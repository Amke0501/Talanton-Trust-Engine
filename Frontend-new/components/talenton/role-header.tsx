...
export function RoleHeader({
  ...
  return (
    <div className="space-y-6">
      {/* Main Top Header Bar */}
      <header className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-[#0d2a1c] px-6 py-4 text-white shadow-xl border border-white/10 backdrop-blur-xl">
        {/* Brand */}
        ...
        {/* Role Switcher Tabs */}
        <div className="flex items-center gap-1.5 rounded-full bg-[#103a27]/80 p-1.5 border border-white/10">
          {roles.map((r) => {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between px-1">
        <div>
          <h2 className="font-serif text-3xl font-bold tracking-tight text-[#103a27]">
            {title}
          </h2>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-xl border border-[#103a27]/20 bg-white px-3.5 py-2 text-xs font-medium text-[#103a27] shadow-sm">
            ...
            <span className="font-mono font-bold text-[#103a27]">{fileReference}</span>
          </div>
          <button
            type="button"
            onClick={() => alert(`Exporting report for ${fileReference}...`)}
            className="flex items-center gap-2 rounded-xl border border-[#103a27]/20 bg-white px-3.5 py-2 text-xs font-semibold text-[#103a27] shadow-sm hover:bg-[#eaf4e5] transition-colors"
          >
